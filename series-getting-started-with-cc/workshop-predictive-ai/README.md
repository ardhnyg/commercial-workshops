...
## <a name="step-8"></a>Connect Flink with AI Model (Optional)
...
7. Now let's invoke the model and get the results.
...
8. The AI Response can then be used to assist human operator in detecting fraudulent transactions.
...
9. Stop the query and observe the responses generated by the AI model.
</details>
...
3. Create your Azure OpenAI Service, if you havent got one :
</br> 
<div align="center">
...
4. Go to your Azure OpenAI Studio. Deploy your model, define the model version and the deployment type. 
</br>
<div align="center">
...
<div align="center">
    <img src="images/azureopenai-4.png" width=100% height=100%>
</div>
</br>
5. Make sure you note the `Target URI Endpoint` and `Key` of your model.
</br>
<div align="center">
    <img src="images/azureopenai-5.png" width=100% height=100%>
</div>
</br>
6. After your model is ready to use, we can create the connection to your model using the endpoint URI and the API Key of the model. Open Confluent CLI, and type in this command : 

```bash
confluent flink connection create azure-openai-connection --cloud azure --region westus2 --type azureopenai --endpoint "<azureopenai endpoint>" --api-key <API Key>
```

7. After creating connection, we need to create the model in Flink before we could invoke on our query.
```sql
CREATE MODEL FraudDetectionModel
INPUT (`details` VARCHAR(2147483647))
OUTPUT (`message` VARCHAR(2147483647))
WITH (
  ...
  'azureopenai.PARAMS.max_tokens' = '20000',
...
10. Stop the query and observe the responses generated by the AI model.
</details>
